...
for my $compdate (sort{$a <=> $b} keys %mongoljunk_tot_pop) {
    ...
    my %data_to_date = ();
    for my $past_date (sort{$b <=> $a} keys %mongoljunk_pop) {
        next if $past_date > $compdate;
        next if $past_date < $comp_N_days_ago;
        for my $vax_group (sort keys %{$mongoljunk_pop{$past_date}}) {
            for my $age_group (sort keys %{$mongoljunk_pop{$past_date}->{$vax_group}}) {
                next unless exists $mongoljunk_pop{$past_date}->{$vax_group}->{$age_group};
                $data_to_date{$age_group}->{$vax_group}->{'alive'} += $mongoljunk_pop{$past_date}->{$vax_group}->{$age_group}->{'alive'};
                $data_to_date{$age_group}->{$vax_group}->{'dead'}  += $mongoljunk_pop{$past_date}->{$vax_group}->{$age_group}->{'dead'};
            }
        }
    }
    ...
    my %daily_asmr = ();
    for my $age_group (sort keys %data_to_date) {
        my $standard_percent = $standard_pop{$age_group} // die;
        for my $vax_group (sort keys %{$data_to_date{$age_group}}) {
            my $vax_group_alive            = $mongoljunk_tot_pop{$compdate}->{$vax_group} // die;
            my $alive_in_age_group         = $data_to_date{$age_group}->{$vax_group}->{'alive'} // die;
            my $age_group_percent_of_total = nearest(0.01, $alive_in_age_group * 100 / $vax_group_alive);
            my $deaths_in_age_group        = $data_to_date{$age_group}->{$vax_group}->{'dead'} // 0;
            $alive_in_age_group            = $alive_in_age_group / ($rolling_days + 1);
            $deaths_in_age_group           = $deaths_in_age_group / ($rolling_days + 1);
            my $deaths_per_100k            = $deaths_in_age_group * 100000 / $alive_in_age_group;
            
            # Calculate the adjusted deaths per 100k (ASMR) for the age group
            my $deaths_per_100k_ASMR       = $deaths_per_100k * $standard_percent / 100 * 365;
